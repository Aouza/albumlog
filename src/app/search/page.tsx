...
export default function SearchPage() {
  const [query, setQuery] = useState("Thornhill");
    ...
    <div className="space-y-5">
      <section className="rounded-[28px] border border-white/10 bg-[#0b0b16]/86 p-6 shadow-[0_24px_90px_rgba(0,0,0,0.45)] backdrop-blur-xl">
        ...
        <p className="mt-2 max-w-2xl text-white/52">
          Pesquise por album, artista ou genero. Nesta versao, os resultados vem do catalogo mockado.
        </p>
      </section>
      ...
      <label className="relative block">
        ...
        <input
          ...
          placeholder="Ex.: The Dark Pool, Thornhill, jazz"
      ...
      {search.data?.length === 0 && (
        <div className="rounded-2xl border border-dashed border-white/20 bg-[#0b0b16]/80 p-8 text-center">
          <h2 className="font-semibold text-white">Nenhum album encontrado</h2>
          <p className="mt-2 text-sm text-white/48">Tente buscar por Thornhill, Opeth ou Miles Davis.</p>
        </div>
      )}
    </div>
  );
